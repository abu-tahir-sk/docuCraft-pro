...
import { FileText, FileSpreadsheet, ShieldCheck, Clock, ArrowRight, Sun, Moon } from 'lucide-react';

export default function DashboardHome() {
  const [darkMode, setDarkMode] = useState(false);

  // ডার্ক মোড টগল করার লজিক
  useEffect(() => {
    if (darkMode) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 p-6 transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          ...
          className="p-3 bg-white dark:bg-slate-800 rounded-full shadow-sm border dark:border-slate-700 text-gray-900 dark:text-yellow-400"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        {cards.map((card, index) => (
          <div key={index} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">{card.title}</p>
                <h2 className="text-3xl font-bold mt-1 text-gray-900 dark:text-white">{card.value}</h2>
              </div>
              <div className={`${card.color} w-12 h-12 rounded-xl text-white flex items-center justify-center`}>
                {card.icon}
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-gray-400 dark:text-gray-500 text-xs uppercase border-b border-gray-100 dark:border-slate-800">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                {[1,2,3].map((i) => (
                  <tr key={i} className="text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <td className="py-4"><span className="px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-md text-[10px]">Paid</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-6">
          ...
          <div className="space-y-4">
            {["New Invoice", "New Quotation", "New Agreement"].map((action, idx) => (
              <button key={idx} className="w-full flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-700 dark:text-gray-200 rounded-xl transition-colors font-semibold">
                {action}
